export const addFoodLog = (food, consumedQt, userJwt) => {
  ...
  foodLogs[currentDate] =
    foodLogs[currentDate] === undefined ? {} : foodLogs[currentDate]
    ...
    fetch(API_URL, options)
      .then(res => {
        if (res.ok) {
          ...
          reject(
            new Error(
              'Validation error. Food is not added your. Please try again'
            )
          )
        } else if (res.status === 401) {
          reject(new Error('token authentication Error. Please Sign in Again.'))
        } else {
